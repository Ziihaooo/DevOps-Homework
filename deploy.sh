...
aws s3 cp s3://$S3_BUCKET/deploy/docker-compose /usr/local/bin/docker-compose --region ap-southeast-2
sudo chmod +x /usr/local/bin/docker-compose

sudo mkdir -p /opt/$PROJECT_NAME
aws s3 cp s3://$S3_BUCKET/deploy/docker-compose.yml /opt/$PROJECT_NAME/docker-compose.yml --region ap-southeast-2
aws s3 cp s3://$S3_BUCKET/deploy/Makefile /opt/$PROJECT_NAME/Makefile --region ap-southeast-2
